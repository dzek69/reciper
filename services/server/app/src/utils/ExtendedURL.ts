import { last } from "bottom-line-utils";
import urlJoin from "url-join";

class ExtendedURL extends URL {
    public constructor(rawUrl: string) {
        if (!rawUrl) {
            throw new Error("ExtendedURL requires an url");
        }
        let url = rawUrl;
    ...
    public clone() {
        return new ExtendedURL(this.href);
    }
    ...
    public createFromRelative(relativeUrl: string | undefined | null) {
        ...
        if (/^[a-z]+:\/\//.exec(relativeUrl)) {
            return new ExtendedURL(relativeUrl);
        }
        if (relativeUrl.startsWith("//")) {
            return new ExtendedURL(this.protocol + relativeUrl);
        }
        if (relativeUrl.startsWith("/")) {
